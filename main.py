...
def send_to_honeypot(payload, connection_id):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            # Connect to the honeypot server on port 8001
            s.connect(("127.0.0.1", 8001))
            src_ip, src_port = s.getsockname()
            if isinstance(payload, str):
                payload = payload.encode('utf-8')
...
def listen_on_port_8001():
    ...
    try:
        with pydivert.WinDivert(filter) as w:
            ...
            for packet in w:
                if packet.is_outbound:
                    # This is a response from the honeypot
                    response_connection_id = f"{packet.dst_addr}:{packet.dst_port}-{packet.src_addr}"
                    original_sender = original_senders.get(response_connection_id)
                    if original_sender:
                        packet.dst_addr, packet.dst_port = original_sender
                        packet.src_port = 8000
                    ...
                    # This is an incoming packet to the honeypot, we don't need to do anything
                    w.send(packet)
    except Exception as e:
